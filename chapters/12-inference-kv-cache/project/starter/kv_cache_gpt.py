"""
KV-Cache Your GPT — STARTER scaffold.
=====================================
Fill the one TODO: implement the KV-cache inside the attention (append the new token's key/value to
the cached past, and save it back). The generators and benchmark are done for you.
Reference: ../solution/kv_cache_gpt.py.
...
class Attn(nn.Module):
    ...
    def forward(self, x, cache, po):
        ...
        if cache is not None:
            # ✍️ TODO: the KV-cache. Append the new k/v onto the cached past (torch.cat, dim=2),
            #          then save them back into the cache so the next step reuses them.
            #          hint:
...
def main():
    ...
    if not torch.equal(a, b):
        raise SystemExit("Cached output differs from naive — implement the ✍️ TODO (the KV-cache "
                         "append/save in Attn.forward), then re-run.")
