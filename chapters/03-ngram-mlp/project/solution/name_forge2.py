"""
The Name Forge MK II — REFERENCE SOLUTION.
==========================================
An MLP-powered character-level name generator with --start and --temperature controls.
...
def main():
    ...
    Xtr, Ytr = build_dataset(words, stoi, block_size)     # train on all the names
    ...
    def sample():
        ...
        while True:
            ...
            if args.temperature != 1.0:                         # temperature reshaping
                probs = probs.clamp_min(1e-9) ** (1.0 / args.temperature)
                probs = probs / probs.sum()
            # ---- TODO #2: sample the next id, then slide the context ----
